...
void array_from_file(WeatherTable array, const char *filepath) {
    ...
    while (!feof(file)) {
        ...
        unsigned int index_y = k_year - FST_YEAR;
        unsigned int index_m = k_month - 1;
        unsigned int index_d = k_day - FST_DAY;
        
        if(index_y > YEARS || index_m > MONTHS || index_d > DAYS){
            fprintf(stderr, "Invalid array.\n");
            exit(EXIT_FAILURE);
        }
        
        array[index_y][index_m][index_d] = weather;
    }
