...
int main(int argc, char *argv[]) {
    ...
    printf("\nMinima temperatura historica: %d\n", min_t);
    
    /*procedimiento maxima temperatura anual*/
    proc_max_anual(array, output);
   
    /*imprimo la funcion*/
   
    for(int i = FST_YEAR; i <= LST_YEAR; i++ ){
        printf("Mayor temperatura del año %d: %d\n",i, output[i]);
    }
    
    /*print procedure*/
    proc_max_pres_m(array, outputp);

    for(int i = FST_YEAR; i <= LST_YEAR; i++ ){
        printf("Mayor mes de precipitacion del año %d: %u\n",i, outputp[i]);
    }
